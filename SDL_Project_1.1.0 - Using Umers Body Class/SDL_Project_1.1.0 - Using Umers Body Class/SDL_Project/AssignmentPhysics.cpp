#include "AssignmentPhysics.h"
#include "Body.h"
#include <SDL.h>
#include "GameManager.h"
#include <SDL_image.h>

AssignmentPhysics::AssignmentPhysics(SDL_Window* sdlWindow_)
{
	window = sdlWindow_;
	elapsedTime = 0.0f;
	gravity = -9.8f;
...
AssignmentPhysics::~AssignmentPhysics() { }

bool AssignmentPhysics::OnCreate()
{
...
void AssignmentPhysics::OnDestroy()
{
	//delete each object
	for (int i = 0; i < NUM_BODIES; i++)
	{
...
void AssignmentPhysics::Update(const float time)
{
...
void AssignmentPhysics::HandleEvents(const SDL_Event &event)
{
	//Look for a keypress 
